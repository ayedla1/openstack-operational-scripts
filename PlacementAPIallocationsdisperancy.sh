...
sleep 1s
source ~/keystonercv3
host=$(cat /etc/hosts |awk /$(hostname)/'{print $1}')
echo " Issuing token...."
...
#echo "curl -X GET -H 'OpenStack-API-Version: placement 1.10' -H 'content-type: application/json' -H 'X-Auth-Token: '"$a"'' http://$host:8778/resource_providers/$b/allocations |python -m json.tool"
echo "BELOW ARE THE ALLOCATIONS FOR COMPUTE   ---------$h--------- "
curl -s -X GET -H 'OpenStack-API-Version: placement 1.10' -H 'content-type: application/json' -H 'X-Auth-Token: '"$a"'' http://$host:8778/resource_providers/$b1/allocations |python -m json.tool| grep -i -B1 resources | grep "[0-9]" | sed 's/.\{3\}$//g' | sed 's/"//g' | sort -n
count1=$(curl -s -X GET -H 'OpenStack-API-Version: placement 1.10' -H 'content-type: application/json' -H 'X-Auth-Token: '"$a"'' http://$host:8778/resource_providers/$b1/allocations |python -m json.tool| grep -i -B1 resources | grep "[0-9]" | sed 's/.\{3\}$//g' | sed 's/"//g'|wc -l)
...
#diff <(curl -s -X GET -H 'OpenStack-API-Version: placement 1.10' -H 'content-type: application/json' -H 'X-Auth-Token: '"$a"'' http://$host:8778/resource_providers/$b1/allocations |python -m json.tool| grep -i -B1 resources | grep "[0-9]" | sed 's/.\{3\}$//g' | sed 's/"//g' | sort -n) <(nova hypervisor-servers $c | awk '{print $2}' |sed 2d | sort -n)
...
curl -s -X GET -H 'OpenStack-API-Version: placement 1.10' -H 'content-type: application/json' -H 'X-Auth-Token: '"$a"'' http://$host:8778/resource_providers/$b1/allocations |python -m json.tool |grep -A4 $d
...
curl -s -X  PUT http://$host:8778/allocations/$d -H 'OpenStack-API-Version: placement 1.10' -H 'content-type: application/json' -H 'X-Auth-Token: '"$a"'' -d '{ "allocations": [{"resource_provider": {"uuid": "'$uuid'"}, "resources": {"VCPU": '$VCPU', "MEMORY_MB": '$MEMORY_MB', "DISK_GB": '$DISK_GB'}}], "project_id": "'$project_id'", "user_id":"'$user_id'"}'
